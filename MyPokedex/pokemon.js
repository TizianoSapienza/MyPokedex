...
class Pokedex {
  ...
  async fetchPokemons() {
    ...
    this.allPokemons = data.results.map((pokemon, index) => new Pokemon(index + 1, pokemon.name, pokemon.url));
  }
  ...
  handleSearch() {
    ...
    if (this.numberFilter.checked) {
      filteredPokemons = this.allPokemons.filter((pokemon) => pokemon.id.toString().startsWith(searchTerm));
    } else if (this.nameFilter.checked) {
      filteredPokemons = this.allPokemons.filter((pokemon) => pokemon.name.toLowerCase().startsWith(searchTerm));
    } else {
    ...
    this.notFoundMessage.style.display = filteredPokemons.length === 0 ? 'block' : 'none';
  }
